...
# 使用skimage.measure.label 将相同类别的像素聚合为对象
labeled_image = measure.label(clustered_image, connectivity=2)  # 聚合同一类像素成连通区域
...
# 聚类结果对象
plt.subplot(1, 2, 2)
plt.title("K-means Clustering with Objects")
plt.imshow(labeled_image, cmap='tab10')  # 使用'jet'色图来显示聚类后的区域
